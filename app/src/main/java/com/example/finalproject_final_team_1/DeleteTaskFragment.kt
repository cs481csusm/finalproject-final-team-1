...
class DeleteTaskFragment : Fragment(R.layout.fragment_delete_task) {
     ...
     */
    private fun deleteTask(name: String, day: Int, hours: Int){
        val database = FirebaseFirestore.getInstance()
        val dbReference = database.collection("task").whereEqualTo("name", name).whereEqualTo("day",day).get().addOnSuccessListener {
            for(document in it ){
